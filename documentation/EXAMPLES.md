...
Two things to read here. Every registered check appears, so `false` records
"scanned, not safe to enforce here" rather than "not considered". And an
...
The SCP and RCP directories each get their own copy, generated once and shared
by every module call in that directory.
...
**File**: `test_environment/headroom_results/scps/deny_ec2_imds_v1/fort-knox.json`
...
**File**: `test_environment/headroom_results/rcps/deny_sts_third_party_assumerole/acme-co.json`
...
account produced a finding. The Terraform files that exist are the ones a safe
placement was found for.
