...
A finished run leaves the directory layout
[`EXAMPLES.md`](EXAMPLES.md#directory-layout-after-a-run) shows: one result
file per check per analyzed account under `headroom_results/`, and one
Terraform file per placement target under `scps/` and `rcps/`.
[`spec/contracts/results.md`](../spec/contracts/results.md) is the authority on
the result files and
[`spec/contracts/terraform.md`](../spec/contracts/terraform.md) on the
Terraform; what follows is the operator's view of the second.

Each run writes the **complete** desired state of the two Terraform
directories. A file Headroom generated for a target that no longer has a
placement is deleted, so a policy that moves from an OU down to individual
accounts does not leave the OU-wide attachment behind still enforcing it.
Headroom's own files open with `# Code generated by Headroom. DO NOT EDIT.`;
anything without that line is never touched, and
["Refusing to apply"](#refusing-to-apply) below lists exactly what that
covers. A run that reads no result files aborts rather than emptying the
directories.

Nothing is written until the whole run has been rendered and checked: results
are parsed, policies placed, every file rendered, the plan validated, and every
destination proven to be Headroom's before the first file is touched. A failure
in any of that leaves the previous run's output exactly as it was. Past that
point the guarantee is per-file rather than whole-run;
["Filesystem Error during Terraform generation"](#filesystem-error-during-terraform-generation)
below describes it.
...
no default of its own: a value-taking option you omit arrives as `None`,
`merge_configs` drops it, and the YAML file or the `config.py` field default
decides. `--exclude-account-ids` is the one boolean flag, so from the CLI it
can only turn redaction on: omit it and the YAML value stands, and there is no
`--no-exclude-account-ids` to override `exclude_account_ids: true`. Every
