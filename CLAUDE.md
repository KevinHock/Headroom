...
- **INV-13** — collection, writing, parsing, and placement are driven by the registry. Terraform generation is not yet: `RCP_TERRAFORM_VARIABLES` and `_build_scp_terraform_module` both name checks by hand, and a test guards each. Check discovery in `headroom/checks/__init__.py` is the one sanctioned dynamic import; everywhere else, import at the top of the file.
