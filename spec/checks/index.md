...
| # | Where | Conflict |
|---|---|---|
| 1 | [`deny_iam_saml_provider_not_aws_sso`](scps/deny_iam_saml_provider_not_aws_sso.md) | The summary omits the `violations` count, so placement reads zero and can recommend the deny at root for an account this check just found non-compliant. Contradicts the safety promise in [`../product.md`](../product.md). |
...
| 4 | ECR, KMS, Secrets Manager, SQS | A `Federated` or `CanonicalUser` principal aborts the whole run. It carries no account ID and so blocks the account, exactly as a wildcard does — which is how [`deny_s3_third_party_access`](rcps/deny_s3_third_party_access.md) records it. Four checks stop the run where one reports. |
