...
Everywhere else, imports are at the top of the file. It exists so a new check
needs no edit anywhere else — the registry, not a hardcoded list, is what every
later stage iterates (INV-13).
...
Every check is constructed with the same keyword arguments:

`check_name`, `account_name`, `account_id`, `results_dir`, `org_account_ids`,
`exclude_account_ids`.

`BaseCheck.__init__` accepts and ignores `**kwargs`, so a check that needs none
of the extras — `org_account_ids` is only meaningful to RCP checks — does not
have to declare it. A check needing something genuinely new must have it passed
by `run_checks_for_type`, which constructs every check identically.
...
Steps 4 and 5 are the two places the registry cannot wire a check up on its own.
